...
(defmethod print-object ((object token) stream)
  (print-unreadable-object (object stream :type t :identity nil)
    (with-slots (kind value start) object
      (format stream "~s ~s @ ~d" value kind start))))
       ...
       (and
        ...
        ,@(if (is-token parser value)
              `((progn (vector-push-extend
                        (make-instance 'token :kind 'string-literal :value ,value :start start) 
                        output) t))))
    ...
    (if (and (or save-p (is-token parser name)) (not (eql save-p 'not)))
        `(and (,name :save-p t)
          ...
          ,@(when (is-token parser name)
                  `((progn
                      (vector-push-extend
                       (make-instance 'token :kind ',name :value last-match :start start)
                       output)
                      t))))
        `(,name))))
