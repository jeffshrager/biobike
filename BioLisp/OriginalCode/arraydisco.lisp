...
(defun correlate-genes (a b)
  (or (gethash (cons a b) *correlation-cache*)
      ...
      (let ((r (correlate (gethash a *hihara-table*)
...
(defun find-all-paths (model)
  (let ((genes (model-genes model))
    ...
    (loop for start in genes
	  append (loop for target in genes
		       ;; We have a special case for self-edges because otherwise we'll
		       ;; always get them (since the test in trace-paths) just looks for (eq start target)
		       ;; which is always true for self-edges!)
		       if (eq start target)
		       append (when (gethash target (gethash start table)) (list (list start)))
		       else append (trace-paths table start target)))))

;;; This is the heart of the operation.  Just follow all the ways out of every
;;; node until you reach the target in all possible ways, collecting paths in the
;;; optional collector var (path) as you go. When you hit the end, return the collector.

(defun trace-paths (table here target &optional path)
  (cond ((eq here target) (list (cons here path)))
	(t (loop for next being the hash-keys of (gethash here table)
		  using (hash-value +-)
		  when +- ; Otherwise we'll be including entries in the hash table that contain nil!
		  append (trace-paths table next target (cons here path))))))

;;; Here's a test function:

(defun test ()
  (setq m (create-empty-model '(a b c d e f g)))
  (code-model m '((+ a b) (- a c) (+ b d) (- c e) (+ c f) (- b f) (- e f) (+ f g)))
...
(defun trace-paths (table here target &optional path)
  (cond ((eq here target) (list (cons here path)))
	(t (loop for next being the hash-keys of (gethash here table)
		 using (hash-value +-)
		 when +- ; Otherwise we'll be including entries in the hash table that contain nil!
		 ;; *********** Cons the edge polatiry in as well. ***********
...
(defun test2 ()
  (setq m (create-empty-model '(a b c d e f g)))
  (code-model m '((+ a b) (- a c) (+ b d) (- c e) (+ c f) (- c c) (- b f) (- e f) (+ e b) (+ f g)))
  (mapcar #'print (find-all-paths m)))

(defun improve (model-expression)
  (let* ((model (create-empty-model (loop for (+- a b) in model-expression
					  with names = nil
					  do 
					  (pushnew a names)
					  (pushnew b names)
    ...
    (loop with current-score = (score-model model)
	  ...
	  (let ((new-score (score-model model)))
	    (format t "Resulting mode score is ~a~%" new-score)
...
;;; This needs a special case for self-paths, which look like this: ((-+ . gene))
    ...
    (loop with current-score = initial-score
          ...
          do (loop for to-vertex in vertices
                   ...
                   do (loop for new-edge-type in edge-types
				     ...
				     ;; Ensure that if we have to reset the edge, it gets set to 
				     ;; the best last type, rather than a previous, possibly older value.
...
(mapcar #'compile '(find-all-paths trace-paths))

;;; This version uses the "pushnewt" method to efficiently remove duplicate
;;; paths, plus does a rather hairy version of subpath caching.  Actually, it could
;;; be made even more efficient by not reporting paths at all, but instead just
;;; returning the polarities and path lengths for scoring!
  ...
  (let ((genes (model-genes model))
    ...
    (loop for start in genes
	  do (loop for target in genes
			 ;; We have a special case for self-edges becasue otherwise we'll
			 ;; always get them (since the test in trace-paths) just looks for (eq start target)
			 ;; which is always true for self-edges!)
			 ...
			 (loop for path in 
			       (cond (cached-paths)
				     ...
				     (t (let ((paths (trace-paths table start target)))
					  ...
					  paths)))
			       do (cond ((gethash path *path-pushnew-table*))
...
;;; Note that you need to extract the terminal element from its polarity embedding,
;;; that is, (+ . G) -> G if it's the last element in the path.  Note that the paths are
;;; reversed to head are tails, and v.v.!  This could prob. be improved!
